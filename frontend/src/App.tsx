...
export default function App() {
  const [session, setSession] = useState<{ token: string; user: User } | null>(() => { const saved = localStorage.getItem(SESSION_KEY); return saved ? JSON.parse(saved) : null; });
  const [tasks, setTasks] = useState<Task[]>([]); const [rooms, setRooms] = useState<Room[]>([]); const [selectedRoom, setSelectedRoom] = useState<Room | null>(null);
  const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null);
  async function loadRooms() { if (session) setRooms(await fetchRooms(session.token)); }
  async function loadTasks() { if (!session) return; try { setLoading(true); setTasks(await fetchTasks(session.token, selectedRoom?.id)); setError(null); } catch { setError("Görevler yüklenemedi. Oturumunuz sona ermiş olabilir."); } finally { setLoading(false); } }
  useEffect(() => { if (session) { loadRooms().catch(() => setError("Odalar yüklenemedi.")); } }, [session]);
  useEffect(() => { loadTasks(); }, [session, selectedRoom]);
  useEffect(() => { if (!session || !("Notification" in window)) return; const timer = window.setInterval(() => tasks.forEach((task) => { if (task.reminderAt && task.status === "pending" && new Date(task.reminderAt).getTime() <= Date.now() && !sessionStorage.getItem(`reminded-${task.id}`)) { if (Notification.permission === "granted") new Notification("Görev hatırlatıcısı", { body: task.title }); sessionStorage.setItem(`reminded-${task.id}`, "true"); } }), 30000); return () => window.clearInterval(timer); }, [tasks, session]);
  function completeAuth(token: string, user: User) { const next = { token, user }; localStorage.setItem(SESSION_KEY, JSON.stringify(next)); setSession(next); }
  async function handleCreate(title: string, description: string, isImportant: boolean, reminderAt: string | null) { if (!session) return; const task = await createTask(session.token, { title, description, isImportant, reminderAt: reminderAt ? new Date(reminderAt).toISOString() : null, roomId: selectedRoom?.id ?? null }); setTasks((prev) => [task, ...prev]); if (reminderAt && "Notification" in window && Notification.permission === "default") await Notification.requestPermission(); loadRooms(); }
  async function handleStatusChange(id: number, status: Task["status"]) { if (!session) return; const updated = await updateTaskStatus(session.token, id, status); setTasks((prev) => prev.map((task) => task.id === id ? updated : task)); }
  async function handleDelete(id: number) { if (!session) return; await deleteTask(session.token, id); setTasks((prev) => prev.filter((task) => task.id !== id)); loadRooms(); }
