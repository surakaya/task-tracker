...
export default function App() {
  ...
  async function loadRooms() {
    if (session) {
      setRooms(await fetchRooms(session.token));
    }
  }
  ...
  async function loadTasks() {
    ...
    try {
      ...
      setError(
        "Görevler yüklenemedi. Oturumunuz sona ermiş olabilir."
      );
    } finally {
      setLoading(false);
    }
    ...
    const timer = window.setInterval(() => {
      tasks.forEach((task) => {
        if (
          ...
          !sessionStorage.getItem(`reminded-${task.id}`)
        ) {
  ...
  async function handleCreate(
    ...
    const task = await createTask(session.token, {
      title,
      description,
      isImportant,
  ...
  async function handleLeaveRoom(room: Room) {
    if (
      !session ||
      !confirm(
        `“${room.name}” odasından ayrılmak istiyor musun?`
      )
    ) {
      return;
    }
  ...
  async function handleRemoveMember(userId: number) {
    ...
    if (
      !confirm(
        `“${member.user.name}” kullanıcısını odadan çıkarmak istiyor musun?`
      )
    ) {
      return;
    }
  ...
  const scopeName =
    selectedRoom?.name ?? "Kişisel alanım";

  return (
    <div className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white/85 backdrop-blur px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="grid place-items-center size-9 rounded-xl bg-brand-600 text-white font-bold">
              ✓
            </div>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[.16em] text-brand-600">
                  Çalışma alanı
                </p>
            ...
            <section className="grid md:grid-cols-[1.6fr_1fr] gap-4">
              <div className="rounded-2xl bg-slate-900 p-5 text-white overflow-hidden relative">
                <div className="absolute -right-10 -top-12 size-40 rounded-full bg-brand-500/30 blur-2xl" />

                <p className="text-xs uppercase tracking-[.18em] text-brand-100">
                  Akış özeti
                </p>

                <h3 className="mt-2 text-xl font-semibold">
                  Odaklan, ilerle, tamamla.
                </h3>
                ...
                <p className="mt-2 text-sm text-slate-300">
                  ...
                  {inProgress.length} görev akışta
                </p>
              </div>
